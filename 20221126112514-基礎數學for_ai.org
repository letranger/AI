...
- 當銀行半年利率50%，每次的利率為 $\frac{1}{2}$，則一年後本利和為 $a(1+\frac{1}{2})(1+\frac{1}{2}) = a(1+\frac{1}{2})^2$
- 當銀行每季利率25%，每次的利率為 $\frac{1}{4}$，則一年後本利和為 $a(1+\frac{1}{4})(1+\frac{1}{4})(1+\frac{1}{4})(1+\frac{1}{4})=a(1+\frac{1}{4})^4$
...
* 指數與對數
** 指數函數
指數函數以 $e^x$ 或 $exp(x)$ 表示，即 $y=e^x=exp(x)$，當x=1時，$y=e^1=2.71828$
** 對數函數
- 以2為底: 若 $2^y=x$ 則 $y=\log_2x$，當 x=1時， $y=\log_2 1=0$
- 以e為底: $y=\log_e x$，當$x=1$ 時，$y=\log_e 1=0$
...
如圖[[fig:numDiff-1]]，若要求曲線在某一點(\(x=5\))的斜率，可以先畫出一條曲線上通過\(x=5\)的割線，此割線通過曲線上的兩個點(\(x=5, x=5+h\))，透過這兩點可求出該割線之斜率，若將\(h\)的值最小化，即可求出曲線在點\(x=5\)上的斜率，此即微分的概念。
...
想像一個連續平滑函數\(f(x)=y\)，即，\(f\)將一個實數\(x\)映射(mapping)到另一個實數\(y\)，因為函數是連續實數，所以在 x 的微小變化也只會導致 y 的微小變化。假設 x 增加了一個很小的值\(\epsilon_x\)，導致 y 做了\(\epsilon_y\)的改變，其數學式可以寫成：
...
除上述 SGD 模式外，SGD 尚存在許多變體，常見的有：momentum SGD, Adagrad, RMSProp 等，這些 SGD 我們稱為最佳化方法(optimization methods)或優化器(optimizers)。
...
- \( \frac{\partial f}{\partial y}\vert_{x_0,y_0} = \lim\limits_{\Delta y \to 0}\frac{f(x_0, y_0+\Delta y)-f(x_0, y_0)}{\Delta y}\)，以\(y\)為變數，固定變數 x，對函數\(f\)做偏微。
...
若曲面函數為\(f(x_0,x_1)=x_0^2+x_1^2\)，當\(x_0\)=3、\(x_1\)=0 時，計算\(x_0\)的偏微分\(\frac{\partial f}{\partial x_0}\)方式如下：
#+BEGIN_SRC python -n -r :results output :exports both
  def numerical_diff(f,x):
      h = 1e-4
      return (f(x+h) - f(x-h))/(2*h)
...
回顧微分的定義：\(f'(h)=\lim\limits_{x \to h}\frac{f(x)-f(h)}{x-h} \)，我們希望透過逐步的調整(增加或減少)\(a\)的值來降低誤差，在這樣的狀況中，由於只有一個變數，所以可以求算導數，讓導數為 0，求得最小值。
...
** 練習一：數值微分——電腦算的跟手算一樣準嗎？

數值微分的重點：不用手算微分公式，只要把 $h$ 取得夠小，用 $\frac{f(x+h) - f(x-h)}{2h}$ 就能逼近真正的導數值。

以下範例計算 $f(x) = x^2 - 6x + 10$ 在 $x=1$ 處的斜率，並畫出切線：
...
x0 = 1
slope = numerical_diff(f, x0)
print(f"數值微分: f'({x0}) = {slope:.4f}")

# 手算驗證: f'(x) = 2x - 6
exact = 2 * x0 - 6
print(f"手算公式: f'({x0}) = {exact}")
print(f"差距: {abs(slope - exact):.10f}")

# 畫圖
x = np.linspace(-1, 7, 100)
tangent = slope * (x - x0) + f(x0)
plt.figure(figsize=(8, 5))
plt.plot(x, f(x), label='$f(x) = x^2 - 6x + 10$')
plt.plot(x, tangent, '--', label=f'切線 (斜率={slope:.1f})')
...
plt.grid(True)
plt.ylim(-2, 20)
plt.savefig("images/diff_demo.png", dpi=300)
print("圖片已儲存")
#+end_src

*** 你的任務
把上面的程式碼複製過來，做以下三個修改：
1. 函數改為 $f(x) = x^3 - 2x^2 + 3x - 1$
2. 求 $x=2$ 處的斜率（改 =x0=）
3. 手算驗證公式改為 $f'(x) = 3x^2 - 4x + 3$（提示：$x^3$ 微分後是 $3x^2$，$-2x^2$ 微分後是 $-4x$，以此類推）
...
# TODO: 畫出新函數的曲線與切線（xlim 建議改成 -1 到 4）
#+end_src
...
手算公式: f'(2) = 7
#+end_example
如果你的兩個結果一致（差距 < 0.001），代表數值微分成功逼近了真正的導數。
...
tangent = slope * (x - x0) + f(x0)
plt.figure(figsize=(8, 5))
plt.plot(x, f(x), label='$f(x) = x^3 - 2x^2 + 3x - 1$')
plt.plot(x, tangent, '--', label=f'切線 (斜率={slope:.1f})')
plt.plot(x0, f(x0), 'ro', markersize=8)
plt.xlabel('x')
plt.ylabel('f(x)')
...
plt.grid(True)
plt.ylim(-5, 30)
plt.savefig("images/diff_exercise.png", dpi=300)
print("圖片已儲存")
#+end_src
...
** 練習二：梯度下降法——在哪一天買機票最便宜？

你打算寒假出國玩。根據學長的血淚經驗，機票價格跟「出發前幾天購買」有關：太早買價格還沒降，太晚買價格飆上天。

以下範例用梯度下降法，找出 $price(x) = (x - 4)^2 + 5$ 的最低價（$x$ = 出發前第幾天）：
...
for i in range(20):
    ...
    history.append(x)
    if i < 5:
        print(f"第{i+1}次: x={x:.4f}, 票價={price(x):.2f}")

print(f"\n最低價在出發前 {x:.1f} 天, 票價 {price(x):.2f} 元")

# 畫圖
t = np.linspace(-1, 9, 100)
plt.figure(figsize=(8, 5))
plt.plot(t, price(t), label='票價曲線')
plt.plot(history, [price(h) for h in history], 'ro-', markersize=4, label='搜尋路徑')
plt.xlabel('出發前天數')
plt.ylabel('票價')
plt.legend()
plt.grid(True)
plt.savefig("images/gd_demo.png", dpi=300)
print("圖片已儲存")
#+end_src
...
請修改上面的程式碼：
1. =price(x)= 改為新函數
2. =price_grad(x)= 改為新函數的導數（提示：$price'(x) = 4x - 20$）
3. 起始點改為 =x = 12.0=（你到第 12 天才想到要查票價）
4. 迭代次數改為 30 次
...
*正確答案：*
#+begin_example
第1次: x=9.2000, 票價=65.28
第2次: x=7.5200, 票價=42.70
第3次: x=6.5120, 票價=34.57
第4次: x=5.9072, 票價=31.65
第5次: x=5.5443, 票價=30.59

最低價在出發前 5.0 天, 票價 30.00 元
#+end_example

手算驗證：$price'(x) = 4x - 20 = 0 \Rightarrow x=5$，$price(5) = 50 - 100 + 80 = 30$

*** 加碼：學習率炸裂實驗
學習率設太大會怎樣？把你寫好的程式複製一份，分別用 =lr = 0.01, 0.1, 0.3, 0.5= 跑一次，比較收斂情形：

#+begin_src python -n -r :results output :exports both
import numpy as np

# TODO: 填入你寫好的 price 和 price_grad

for lr in [0.01, 0.1, 0.3, 0.5]:
    x = 12.0
    for i in range(30):
        x = x - lr * price_grad(x)
    print(f"lr={lr}: x={x:.4f}, 票價={price(x):.2f}")
#+end_src

*你應該看到：*
#+begin_example
lr=0.01: x=7.0576, 票價=38.47  ← 30 次還沒到，龜速
lr=0.1:  x=5.0000, 票價=30.00  ← 剛好
lr=0.3:  x=5.0000, 票價=30.00  ← 收斂超快
lr=0.5:  x=12.0000, 票價=128.00 ← 在 -2 和 12 之間永遠彈跳
#+end_example
這就是為什麼調學習率是訓練神經網路最頭痛（也最重要）的事之一。太小等到天荒地老，太大直接爆炸。
...
plt.legend()
plt.grid(True)
plt.savefig("images/gd_ticket.png", dpi=300)

# 加碼：學習率比較
print("\n=== 學習率比較 ===")
fig, axes = plt.subplots(2, 2, figsize=(12, 8))
for ax, lr in zip(axes.flat, [0.01, 0.1, 0.3, 0.5]):
    x = 12.0
    history = [x]
    for i in range(30):
        x = x - lr * price_grad(x)
...
plt.tight_layout()
plt.savefig("images/gd_lr_compare.png", dpi=300)
print("圖片已儲存")
#+end_src
...
:
: === 學習率比較 ===
: lr=0.01: x=7.0576, 票價=38.47
: lr=0.1: x=5.0000, 票價=30.00
: lr=0.3: x=5.0000, 票價=30.00
: lr=0.5: x=12.0000, 票價=128.00
