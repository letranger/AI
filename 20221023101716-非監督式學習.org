...
*** 異常檢測與新穎檢測（Anomaly & Novelty Detection）
...
# ## 步驟式提取主成分（Principal Components）

# 讀取葡萄酒數據集，數據來自UCI機器學習庫
# 這是一個多分類問題，目標是預測葡萄酒的種類（class label）
df_wine = pd.read_csv('https://archive.ics.uci.edu/ml/'
                        'machine-learning-databases/wine/wine.data',
                        header=None)

# 設定資料欄位名稱
# 第一欄是類別標籤，其餘欄位是葡萄酒的各種特徵值
...
# 將數據分為特徵矩陣 X 和目標向量 y
# X 包含從第二列到最後一列的數據（特徵值）
# y 包含第一列的數據（類別標籤）
X, y = df_wine.iloc[:, 1:].values, df_wine.iloc[:, 0].values

# 將數據分為訓練集（70%）和測試集（30%）
# stratify=y 確保類別分佈在訓練集和測試集中大致相同
X_train, X_test, y_train, y_test = train_test_split(X, y,
                                     test_size=0.3,
                                     stratify=y, random_state=0)

# 1. 對數據進行標準化處理
# 標準化是 PCA 的必要步驟，確保所有特徵值的範圍一致，避免尺度差異影響結果
sc = StandardScaler()
X_train_std = sc.fit_transform(X_train)  # 使用訓練集計算均值與標準差，並標準化數據
X_test_std = sc.transform(X_test)       # 使用訓練集的均值與標準差標準化測試集

# 2. 計算共變數矩陣並進行特徵值分解（Eigendecomposition）
# 共變數矩陣用於衡量特徵之間的相關性
cov_mat = np.cov(X_train_std.T)  # 計算轉置後數據的共變數矩陣

# 計算特徵值（eigenvalues）和特徵向量（eigenvectors）
# 特徵值表示對應主成分的變異數，特徵向量表示主成分的方向
...
plt.ylabel('Explained variance ratio')  # y 軸標籤：解釋變異數比例
plt.xlabel('Principal component index')  # x 軸標籤：主成分索引
plt.legend(loc='best')  # 顯示圖例
plt.tight_layout()  # 自動調整布局
plt.savefig('05_02.png', dpi=300)  # 保存圖片為高解析度 PNG
