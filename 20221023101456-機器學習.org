...
* 機器是如何學習的
** 傳統程式設計
假設有一個用來描述直線的函數: $x=Wx+B$，直線上的每個點都可以用 $x$ 值乘以$W$(權重值)再加上$B$(偏差值)，得到相應的 $y$ 值。
現在假設直線上有兩個點，分別是 $x=w, y=3$ 和 $x=3, y=5$，要如何計算出連接這兩個點的直線所相應的 $W$ 值與 $B$ 值呢？
...
** 機器學習的策略
如果要以機器學習的方式來找出前例中的 $W$ 和$B$ 值，其策略大致如下：
1. 第一步：猜答案
...
** 實作1
如果多加入一個線上的點(4, 7)，以三點當成標準答案來進行預測，其實作的程式碼為：
#+begin_src python -r -n :results output :exports both
...
#+RESULTS:
: [[17.07221]]
: Here is what I learned: [array([[1.7164488]], dtype=float32), array([-0.09227743], dtype=float32)]

預測結果為(1.7164488, -0.9227743)，與正確答案(2, -1)相近。

** 實作2
如果再多加入兩個線上的點(4, 7)，以三點當成標準答案來進行預測，其實作的程式碼為：
#+begin_src python -r -n :results output :exports both
import tensorflow as tf
import numpy as np
from tensorflow.keras import Sequential
...
#+RESULTS:
: [[18.979391]]
: Here is what I learned: [array([[1.9970131]], dtype=float32), array([-0.99074], dtype=float32)]

預測結果為(1.9970131, -0.99074)，與正確答案(2, -1)已經十分相近。
...
深度學習：Deep Learning,是一種機器學習的技術，由於深度學習在現代機器學習中的比重和價值非常巨大，因此常常將深度學習單獨拿出來說。最初的深度學習網絡是利用神經網絡來解決特徵層分佈的一種學習過程。通常我們瞭解的 DNN（深度神經網絡），CNN（[[id:20221023T101414.457264][卷積神經網路]]），RNN（[[id:263cb433-d0eb-4400-a373-35175c000c01][循環神經網路]]），LSTM（長短期記憶網絡）都是隸屬於深度學習的範疇。也是現代機器學習最常用的一些手段。通過這些手段，深度學習在視覺識別，語音識別，[[id:20221023T101534.642520][自然語言處理]]（NLP）等領域取得了使用傳統機器學習算法所無法取得的成就。
...
:PROPERTIES:
:ID:       124c5a49-e887-4a77-9aef-a4f72c81c315
:END:
