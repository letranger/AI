...
- 機器學習是實現[[id:20221023T101138.945879][人工智慧]]的方法之一，機器學習理論主要是設計和分析一些讓電腦可以自動「學習」 的演算法。機器學習演算法是一類從資料中自動分析獲得規律，並利用規律對未知資料進行預測的演算法。
...
#+END_SRC
#+CAPTION: 機器學習訓練模型的流程
#+LABEL:fig:MLTrainingFlow
#+name: fig:MLTrainingFlow
#+ATTR_LATEX: :width 300
#+ATTR_ORG: :width 300
#+ATTR_HTML: :width 500
...
l0 = Dense(units=1)                        (ref:neuron)
model = Sequential([Input(shape=[1]), l0])  (ref:model)
...
**** 程式解說
1. 上述程式使用tensorflow這個模組為主要預測工具，建立了一個模型(第[[(model)]]行)，這個模型中有一個輸入層和一個只有一個神經元的Dense層(第[[(neuron)]]行)
2. 定義好模型後，我們以SGD(隨機梯度下降法)為優化器、用它來找出最佳參數；以MSE(均方差)做為損失函數，計算模型訓練過程中生成的參數優劣判斷標準(第[[(modelCompile)]]行)，並編譯此模型
3. 編譯好模型，我們就拿手邊僅有的2組資料來訓練模型，訓練500回(第[[(modelFit)]]行)
...
l0 = Dense(units=1)
model = Sequential([Input(shape=[1]), l0])
...
*** 實作3
讓我們壯起膽子再跟上帝多要三個線上的點:(-1, -3)、(0, -1)、(1, 1)，以六點當成標準答案來進行預測(如圖[[fig:solution-4]])：
...
@startuml
 ...
 state 已訓練模型 {
 state 訓練資料 {
 state Xs #palegreen : [-1.0, 0.0, 1.0, 2.0, 3.0, 4.0]
 state Ys #palegreen : [-3.0, -1.0, 1.0, 3.0, 5.0, 7.0]
 }
 state 模型 {
 state l0 #lightgray : 黑盒子
 }
...
#+CAPTION: 機器學習的類型
#+LABEL:fig:MLTypes-overview
#+name: fig:MLTypes-overview
#+ATTR_LATEX: :width 300
#+ATTR_ORG: :width 300
#+ATTR_HTML: :width 500
...
- 為迄今為止最常見的機器學習，泛指一群的機器學習演算法，是從一組「已標記」的「訓練資料集」(training dataset)來學習(訓練)，並導出模型。然後，以此一模型對「未標記」的類似資料進行預測分類，其運作流程如圖[[fig:FlowChartOfSupervisedLearning]][fn:1]所示。典型的例子為早期電子郵件的垃圾信件是讓使用者先去標記某些信為垃圾郵件，然後藉由這些被標記的郵件來推論找出其他可能的垃圾郵件；由此看來，我們以為 Gmail 很好心的提供給我們為信件加註「垃圾」、「廣告」的功能，其實是 Google 利用我們當免費勞工為他們提供信件加註標籤的工作。
*** 方法
...
*** 類型：
**** 分類
基於從訓練集資料觀測到的分類類別來標籤、預測新資料的類別，如上述的垃圾郵件即為典型的二元分類工作(如圖[[fig:01_03]])，類別分類工作也可以進行「多類別分類」(multi-class classification)，如典型的 MNist 手寫數字辨識，即是將手寫的 0-9 數字進行預測辨識，並給出一個 0-9 的類別標籤。
#+CAPTION: 典型的分類
#+name: fig:01_03
...
**** 迴歸
利用輸入資料的「特徵」來預測出一個「值」，例如，根據房屋的地點、坪數、樓層、房間數等變數, 發掘出這些變數之間的關係，進而預測房價，如圖[[fig:01_04]]。
#+CAPTION: 典型的迴歸
#+name: fig:01_04
#+ATTR_LATEX: :width 200
#+ATTR_ORG: :width 200
...
圖[[fig:iris-dt]]為「鳶尾花資料集」(http://archive.ics.uci.edu/ml/datasets/Iris)的分類結果，依鳶尾花的四個特徵：花萼(sepal)長度、寬度、花瓣(petal)長度、寬度，對其進行鳶尾花種類判斷。
...
#+CAPTION: 將超平面最大化
#+LABEL:fig:SVM-maximize-margin
#+name: fig:SVM-maximize-margin
#+ATTR_LATEX: :width 300
#+ATTR_ORG: :width 300
#+ATTR_HTML: :width 400
[[file:images/機器學習的類型/2024-01-31_13-07-55_2024-01-31_13-07-31.png]]

SVM不僅能將資料分門別類，甚至還可以找到最大化的「分離超平面」（類似於三維以上空間中的一個平面）[fn:2]，會最大化每個樣本點與該「超平面」的差。此外，當資料是「不可線性分離」時，支援向量機還可以透過「軟邊界」(soft margin)和「核技巧」(kernel trick)來處理。
...
- 在[[id:20221023T101626.420918][監督式學習]]中，我們事先會知道訓練集資料的正確答案(label)，並依此訓練我們的模型；在[[id:0a5c37c0-741a-4a1a-bec7-f98074830132][強化學習]]的環境中，我們會為代理人定義如何度量特定行動的奬勵；然而，在「非監督式學習」的環境中，我們面對的是未標記類別的資料或未知結構的資料，目的是讓演算法導出結論。最典型的例子就是「集群」(clustering)，即，讓演算法自己根據資料的特性把它們依某種特質分類為不同子集合，這裡的子集合不一定要是有限集合，也可能是無界子集(unbounded subsets)。「受限玻爾茲曼機」以及「深度信念網路」(deep belief networks, DBN)都屬此類。
...
- 階層式分群​​(Hierarchical Clustering​​)
  將資料在一個階層式的樹狀上，反覆的利用拆分以及聚合的方式建立出一個分類系統。階層式分群的優勢在於它使用上的簡單性以及能夠在小資料上操作，然而卻非常難處理大型的資料[fn:5]。
...
- 強化學習的目標在於開發一個系統（或代理人，agent）,他會藉由與環境的互動來改進自身的效能。由於當前的環境狀態資訊通常就包含了所謂「奬勵信號」(reward signal)，強化學習的目的就是找到一個最好的 Policy(策略)，可以讓 reward 最多。所以也可以把「強化學習」視為與「監督式學習」相關聯的領域，然而在強化學習中，環境回饋不能視為真正的事實（或是說，正確的標籤），只能將之視為：測量函數對特定行動所觀測到並回報的一個度量值。如圖[[fig:ReinforcementLearning]]，Agent
...
- 飛的越遠，就會獲得越多的積分——獎勵
#+CAPTION: 強化學習遊戲範例
#+name: fig:ReinforcementLearningGame
#+ATTR_LATEX: :width 500
#+ATTR_ORG: :width 500
#+RESULTS:
...
2. *資料處理（Data Processing）*
  蒐集到的資料往往不夠乾淨，這時要進行資料清理。例如：刪除不必要或重複的特徵、補齊遺漏值，或對資料進行標準化處理。這些步驟可以提升模型學習的效果。
...
圖[[fig:feature-vector]]中的特徵向量表示某個人的特徵，包含了他的身高、體重、年齡等資訊。這些特徵可以用來描述這個人，並與其他人進行比較。
*** Images
...
[fn:7] [[https://www.books.com.tw/products/F014278520][Hands-On Machine Learning with Scikit-Learn, Keras, and Tensorflow: Concepts, Tools, and Techniques to Build Intelligent Systems]]
